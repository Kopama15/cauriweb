...
const SearchResultsPage = () => {
  ...
  }, [query, sort, ...Object.values(filters)]);
    ...
    <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {items.map((item, idx) => (
        <li key={idx} className="border p-2 rounded shadow-sm hover:shadow-md">
          ...
          <p className="text-sm text-gray-600">FCFA {item.price?.toLocaleString()}</p>
        </li>
      ))}
    </ul>
  );
    ...
    <div className="p-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <input type="text" placeholder="Catégorie" value={filters.category}
            onChange={(e) => updateParam('category', e.target.value)}
            className="border px-2 py-1 rounded text-sm" />

          <input type="text" placeholder="Couleur" value={filters.color}
            onChange={(e) => updateParam('color', e.target.value)}
            className="border px-2 py-1 rounded text-sm" />

          <input type="text" placeholder="Taille" value={filters.size}
            onChange={(e) => updateParam('size', e.target.value)}
            className="border px-2 py-1 rounded text-sm" />

          <input type="text" placeholder="Marque" value={filters.brand}
            onChange={(e) => updateParam('brand', e.target.value)}
            className="border px-2 py-1 rounded text-sm" />

          <label className="flex items-center text-sm gap-1">
            <input type="checkbox" checked={filters.withDiscount}
